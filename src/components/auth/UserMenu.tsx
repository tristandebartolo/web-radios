...
export function UserMenu() {
  ...
  if (status === 'loading') {
    return (
      <div className="w-8 h-8 rounded-full bg-[var(--secondary)] animate-pulse" />
    );
  }
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="flex items-center gap-2 p-1 rounded-lg hover:bg-[var(--secondary)] transition-colors"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 rounded-xl glass shadow-xl py-1 z-50 animate-slide-up">
          <div className="px-4 py-3 border-b border-[var(--border)]">
            ...
            {isAdmin && (
              <span className="inline-block mt-1 px-2 py-0.5 text-xs bg-[var(--primary)]/20 text-[var(--primary)] rounded-full">
                Admin
              </span>
            )}
          </div>
          ...
          <div className="py-1">
            <Link
              ...
              className="block px-4 py-2 text-sm text-[var(--foreground)] hover:bg-[var(--secondary)] transition-colors"
            >
              Mon profil
            </Link>
            {isAdmin && (
            ...
            <button
              ...
              className="w-full text-left px-4 py-2 text-sm text-[var(--error)] hover:bg-[var(--secondary)] transition-colors"
            >
              Se déconnecter
            </button>
          </div>
